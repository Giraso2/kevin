...
const statusColor = (s) => {
  ...
  if (s === 'approved' || s === 'resolved' || s === 'active') return { color: '#27ae60', bg: '#e8f5e9' };
  if (s === 'rejected' || s === 'inactive') return { color: '#e74c3c', bg: '#fdecea' };
  if (s === 'reviewing') return { color: '#3498db', bg: '#e3f2fd' };
...
const Field = ({ label, children, required }) => (
  <div style={{ marginBottom: 14 }}>
    <label style={{
      ...
      marginBottom: 5, letterSpacing: .4,
    }}>
...
const StatCard = ({ icon, label, value, sub, accent = '#1a3a5c', bg = '#e8f5e9', onClick }) => (
  ...
  >
    ...
    <div>
      <div style={{
        fontSize: 26, fontWeight: 700, color: '#1a3a5c', lineHeight: 1,
        ...
        <tr style={{ background: '#f7f9fb' }}>
          {cols.map((c, i) => (
            <th key={i} style={{
              ...
              fontWeight: 700, color: '#888', letterSpacing: .8,
...
const SuperAdminDashboard = () => {
  ...
  const [adminForm, setAdminForm] = useState({
    fullName: '', email: '', password: '', phone: '', role: 'academic_admin'
  });
  ...
  const fetchTeachers = () => {
    api('/academic-admin/teachers-list')
      .then(d => setTeachers(Array.isArray(d) ? d : []))
      .catch(() => setTeachers([]));
  };

  const fetchStudents = () => {
    api('/academic-admin/students')
  ...
  const toggleAdminStatus = async (admin) => {
    try {
      const result = await api(`/super-admin/admins/${admin._id}`, {
        method: 'PUT',
        body: JSON.stringify({ isActive: !admin.isActive })
      });
      Swal.fire('✅ Status Updated!', `${admin.fullName} is now ${result.user.isActive ? 'active' : 'inactive'}`, 'success');
      fetchAdmins();
      fetchStats();
    } catch (e) {
  ...
  const permissionAction = async (p, status) => {
    ...
    try {
      ...
      // If approved, offer to print slip
      if (!isReject) {
        ...
        if (printResult.isConfirmed) {
          const token = getToken();
          const slipUrl = `${API_URL}/permissions/${p._id}/slip?token=${encodeURIComponent(token)}`;
  ...
  const sendMassMessage = async () => {
    ...
    try {
      ...
      if (massMessageForm.audience === 'all_parents') {
        ...
        recipients = students;
      } else if (massMessageForm.audience === 'all_admins') {
        recipients = admins;
      ...
      setMassMessageForm({ subject: '', content: '', audience: 'all', messageType: 'email' });
    } catch (e) {
  ...
  const updateSchoolSettings = async () => {
    setSaving(true);
    try {
      await api('/school-settings', { method: 'PUT', body: JSON.stringify(schoolSettings) });
      Swal.fire('✅ Settings Updated!', 'School configuration saved successfully', 'success');
  ...
  const menuItems = [
    ...
    { id: 'settings', label: 'School Settings', icon: 'fas fa-cog' },
  ...
  if (loading) {
    return (
      <div style={{
        ...
        background: 'linear-gradient(135deg,#0d2b42,#1a3a5c)',
  ...
  return (
    <div style={{
      display: 'flex', minHeight: '100vh', background: '#f0f3f8',
      ...
      <aside style={{
        ...
        {/* logo */}
        <div style={{
          padding: '22px 18px', borderBottom: '1px solid rgba(255,255,255,.08)',
          display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0
        }}>
          <div style={{
            width: 40, height: 40, background: '#ffc107', borderRadius: 12,
            display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
          }}>
          ...
          {(sidebarOpen || isMobile) && (
            <div>
              <div style={{ fontFamily: "'Crimson Text', Georgia, serif", fontSize: 16, fontWeight: 600, lineHeight: 1.2 }}>
                LYMAR Portal
              </div>
          ...
          {!isMobile && (
            <button onClick={() => setSidebarOpen(!sidebarOpen)}
              style={{
                ...
                color: 'rgba(255,255,255,.5)', cursor: 'pointer', fontSize: 14, flexShrink: 0
              }}>
        ...
        {/* user */}
        <div style={{
          padding: '16px 18px', borderBottom: '1px solid rgba(255,255,255,.08)',
          display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0
        }}>
        ...
        <div style={{ padding: 14, borderTop: '1px solid rgba(255,255,255,.08)', flexShrink: 0 }}>
          <button onClick={() => { localStorage.clear(); navigate('/portal/login'); }}
            style={{
              ...
              padding: '10px 14px', background: 'rgba(231,76,60,.2)',
              border: '1px solid rgba(231,76,60,.3)', borderRadius: 10,
      ...
      <main style={{
        ...
        transition: 'margin-left .3s', minHeight: '100vh',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: '#aaa', letterSpacing: .5 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button onClick={() => setSettingsModal(true)}
              ...
              <i className="fas fa-cog" style={{ marginRight: 4 }} /> Settings
            </button>
            <button onClick={() => setMassMessageModal(true)}
              style={{
                background: '#3498db', border: 'none', borderRadius: 8,
                padding: '6px 12px', cursor: 'pointer', fontSize: 12, color: 'white',
                transition: 'all .2s'
              }}
            ...
            {unread > 0 && (
              <button onClick={() => setActiveTab('messages')}
                style={{
                  ...
                  cursor: 'pointer', color: '#888', fontSize: 18
                }}>
                <i className="fas fa-bell" />
                <span style={{
                  position: 'absolute', top: -4, right: -4, background: '#e74c3c',
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <Btn onClick={() => setAdminModal(true)} icon="fas fa-user-plus" color="#ffc107" textColor="#1a3a5c">
                    New Admin
                  </Btn>
                  <Btn onClick={() => setAnnoModal(true)} icon="fas fa-bullhorn" color="rgba(255,255,255,.15)" textColor="white">
                    Announce
                  </Btn>
                  <Btn onClick={() => setSettingsModal(true)} icon="fas fa-cog" color="rgba(255,255,255,.15)" textColor="white">
                    Settings
                  </Btn>
                </div>
              </div>
              ...
              <div style={{
                ...
                <StatCard icon="fas fa-user-graduate" label="Students" value={stats.totalStudents || students.length}
                  sub="enrolled & active" accent="#1abc9c" bg="#e0f7fa" />
                <StatCard icon="fas fa-chalkboard-teacher" label="Teachers" value={stats.totalTeachers || teachers.length}
                  sub={`${stats.totalClasses || classes.length} classes`} accent="#e67e22" bg="#fff3e0" />
              </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <h3 style={{
                      margin: 0, fontSize: 14, fontWeight: 600, color: '#1a3a5c',
                      display: 'flex', alignItems: 'center', gap: 8
                    }}>
                      <i className="fas fa-gavel" style={{ color: '#9b59b6' }} /> Pending Discipline
                    </h3>
                    <Badge text={`${disciplineCases.filter(c => c.status === 'pending').length} open`}
                      color="#9b59b6" bg="#f3e5f5" />
                  </div>
                    ...
                    <div key={c._id} style={{
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: '#999', marginTop: 2 }}>
                          {c.category} · {fmt(c.createdAt)}
                        </div>
                      </div>
                      <Btn small onClick={() => disciplineAction(c)} icon="fas fa-hammer" color="#9b59b6">
                        Review
                      </Btn>
                    </div>
                  ))}
                  {disciplineCases.filter(c => c.status === 'pending').length === 0 && (
                    <div style={{ textAlign: 'center', padding: '24px 0', color: '#bbb', fontSize: 13 }}>
                      <i className="fas fa-check-circle" style={{
                        fontSize: 28, marginBottom: 8, display: 'block', color: '#27ae60'
                      }} />
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <Badge text={`${permissions.filter(p => p.status === 'pending').length} open`}
                      color="#3498db" bg="#e3f2fd" />
                  </div>
              ...
              <div style={{
                background: 'white', borderRadius: 16, padding: '20px',
                marginTop: 20, boxShadow: '0 2px 12px rgba(0,0,0,.06)'
              }}>
                <div style={{
                  display: 'flex', justifyContent: 'space-between',
                  ...
                  <div>
                    <h3 style={{ margin: 0, fontSize: 14, color: '#1a3a5c' }}>
                      <i className="fas fa-school" style={{ marginRight: 8, color: '#ffc107' }} />
                      {schoolSettings.schoolName}
                    </h3>
                    ...
                    <p style={{ margin: '2px 0 0', fontSize: 12, color: '#888' }}>
                      Academic Year: {schoolSettings.academicYear} · {schoolSettings.term}
                    </p>
                  </div>
                  <Btn small onClick={() => setSettingsModal(true)} icon="fas fa-edit" color="#1a3a5c">
                    Edit Settings
                  </Btn>
                </div>
              </div>
          ...
          {activeTab === 'admins' && (
            <div>
              <div style={{
                ...
                <div>
                  <h2 style={{
                    margin: 0, fontSize: 20, color: '#1a3a5c',
                    fontFamily: "'Crimson Text', Georgia, serif"
                  }}>System Administrators</h2>
                  <p style={{ margin: '4px 0 0', fontSize: 13, color: '#888' }}>
                    {admins.length} admin accounts registered
                  </p>
                </div>
                <Btn onClick={() => setAdminModal(true)} icon="fas fa-plus" color="#1a3a5c">
                  Add Sub-Admin
                </Btn>
              </div>
              <div style={{
                ...
                <Table
                  ...
                  rows={admins.map(a => (
                    <React.Fragment key={a._id}>
                      ...
                      <TD>
                        <Badge
                          text={a.isActive !== false ? 'Active' : 'Inactive'}
                          color={a.isActive !== false ? '#27ae60' : '#e74c3c'}
                          bg={a.isActive !== false ? '#e8f5e9' : '#fdecea'}
                        />
                      </TD>
                      <TD>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          ...
                          <Btn small icon="fas fa-toggle-on" color="#f39c12"
                            onClick={() => toggleAdminStatus(a)}>
                            {a.isActive !== false ? 'Deactivate' : 'Activate'}
                          </Btn>
                          <Btn small danger icon="fas fa-trash" onClick={() => deleteAdmin(a)}>
                            Delete
                          </Btn>
                        </div>
                      </TD>
          ...
          {activeTab === 'announcements' && (
            <div>
              <div style={{
                ...
                <Btn onClick={() => setAnnoModal(true)} icon="fas fa-bullhorn" color="#1a3a5c">
                  Post Announcement
                </Btn>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {announcements.length === 0 && (
                  <div style={{
                    ...
                    <i className="fas fa-bullhorn" style={{
                      fontSize: 36, marginBottom: 12, display: 'block', opacity: .3
                    }} />
                ...
                {announcements.map((ann) => {
                  ...
                  const pColor = priority === 'urgent' ? '#e74c3c' :
                    priority === 'high' ? '#f39c12' : '#27ae60';
                  const pBg = priority === 'urgent' ? '#fdecea' :
                    priority === 'high' ? '#fff3e0' : '#e8f5e9';
                  ...
                  return (
                    <div key={ann._id} style={{
                      ...
                      <div style={{
                        ...
                        <div>
                          <h3 style={{ margin: 0, fontSize: 15, color: '#1a3a5c' }}>
                            {ann.title || 'Untitled'}
                          </h3>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                            <Badge text={priority} color={pColor} bg={pBg} />
                            <Badge
                              text={ann.audience === 'all' ? 'All Users' : (ann.audience || 'All Users')}
                              color="#888" bg="#f5f5f5"
                            />
                          </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 11, color: '#aaa' }}>
                            {ann.createdAt ? fmt(ann.createdAt) : 'Just now'}
                          </span>
          ...
          {activeTab === 'discipline' && (
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 13, color: '#888' }}>
                  Review and take action on student conduct reports
                </p>
              </div>
                  ...
                  rows={disciplineCases.map(c => (
                    <React.Fragment key={c._id}>
                      <TD>
                        ...
                        <div style={{ fontSize: 11, color: '#aaa' }}>{c.className || ''}</div>
                      </TD>
                      <TD><Badge text={c.category || '—'} color="#9b59b6" bg="#f3e5f5" /></TD>
                      <TD>
                        <div style={{
                          maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis',
                          whiteSpace: 'nowrap', fontSize: 12, color: '#555'
                        }}>
                          {c.description || '—'}
                        </div>
                      </TD>
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                <h2 style={{
                  margin: 0, fontSize: 20, color: '#1a3a5c',
                  fontFamily: "'Crimson Text', Georgia, serif"
                }}>Permission Requests</h2>
                <p style={{ margin: '4px 0 0', fontSize: 13, color: '#888' }}>
                  Approve or reject submitted permission requests
                </p>
              </div>
              <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
                {[
                  ...
                  rows={permissions.map(p => (
                    <React.Fragment key={p._id}>
                      <TD>
                        ...
                        <Badge {...roleBadge(p.requesterRole)} text={roleBadge(p.requesterRole).label} size={10} />
                      </TD>
                      ...
                      <TD>
                        {p.status === 'pending'
                          ...
                          : <span style={{ fontSize: 11, color: '#aaa' }}>
                              {p.rejectionReason ? 'Rejected: ' + p.rejectionReason.substring(0, 30) : p.status}
                            </span>
                        }
                        {p.status === 'approved' && (
                          <Btn small icon="fas fa-print" color="#3498db" style={{ marginLeft: 4 }}
                            onClick={() => {
                              const token = getToken();
                              window.open(`${API_URL}/permissions/${p._id}/slip?token=${encodeURIComponent(token)}`, '_blank');
                            }}>
                            Print
                          </Btn>
                        )}
              ...
              <div style={{
                ...
                {['inbox', 'compose'].map(t => (
                  <button key={t} onClick={() => {
                    setMsgTab(t);
                    if (t === 'compose') { setSelectedUser(null); setMessages([]); }
                  }}
                  style={{
                    padding: '7px 18px', borderRadius: 30, border: 'none',
                    cursor: 'pointer', fontSize: 13, fontWeight: 600,
                    background: msgTab === t ? '#1a3a5c' : '#f0f3f8',
                    color: msgTab === t ? 'white' : '#666',
                    transition: 'all .2s',
                  }}>
                    {t === 'inbox'
                      ? <><i className="fas fa-inbox" style={{ marginRight: 6 }} />Inbox
                        {unread > 0 && <span style={{
                          marginLeft: 6, background: '#e74c3c', color: 'white',
                          borderRadius: 20, fontSize: 10, padding: '1px 7px'
                        }}>{unread}</span>}</>
                      : <><i className="fas fa-pen" style={{ marginRight: 6 }} />New Message</>
                    }
                  </button>
                ))}
                <Btn small onClick={() => setMassMessageModal(true)} icon="fas fa-users" color="#3498db">
                  Mass Message
                </Btn>
              </div>
              ...
              {msgTab === 'inbox' ? (
                <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                  ...
                  <div style={{
                    ...
                    display: 'flex', flexDirection: 'column',
                    background: '#fafbff', flexShrink: 0, overflow: 'hidden'
                  }}>
                    <div style={{ padding: '12px 14px', borderBottom: '1px solid #eee' }}>
                      <div style={{ position: 'relative' }}>
                        <i className="fas fa-search" style={{
                          position: 'absolute', left: 10, top: '50%',
                          transform: 'translateY(-50%)', color: '#ccc', fontSize: 12
                        }} />
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                      {filteredUsers.length === 0 && (
                        <div style={{ textAlign: 'center', padding: 30, color: '#ccc', fontSize: 13 }}>
                          No users found
                        </div>
                      )}
                      {filteredUsers.map(u => (
                        <div key={u._id} onClick={() => { setSelectedUser(u); fetchConversation(u._id); }}
                          ...
                          <div style={{ overflow: 'hidden' }}>
                            ...
                            <div style={{
                              fontSize: 10, color: '#ffc107', fontWeight: 700, letterSpacing: .3
                            }}>{roleBadge(u.role).label}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      <>
                        <div style={{
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 11, color: '#ffc107', fontWeight: 700 }}>
                          ...
                          {messages.length === 0 && (
                            <div style={{ textAlign: 'center', color: '#ccc', paddingTop: 40 }}>
                              <i className="fas fa-comments" style={{
                                fontSize: 32, marginBottom: 8, display: 'block'
                              }} />
                        ...
                        <div style={{
                          ...
                          display: 'flex', gap: 10, background: 'white', alignItems: 'flex-end'
                        }}>
                          <textarea value={msgText} onChange={e => setMsgText(e.target.value)}
                            ...
                            style={{
                              ...
                              borderRadius: 12, resize: 'none', fontFamily: 'inherit',
                              fontSize: 13, outline: 'none', boxSizing: 'border-box'
                            }} />
                          <button onClick={sendMessage} disabled={!msgText.trim()}
                            style={{
                              width: 42, height: 42,
                              background: msgText.trim() ? '#1a3a5c' : '#ddd',
                ...
                /* compose */
                <div style={{
                  flex: 1, padding: 28, maxWidth: 600,
                  margin: '0 auto', width: '100%', overflowY: 'auto'
                }}>
                  <h3 style={{
                    margin: '0 0 20px', color: '#1a3a5c',
                    fontFamily: "'Crimson Text', Georgia, serif"
                  }}>New Message</h3>
                    ...
                    <Select value={selectedUser?._id || ''} onChange={e => {
                      ...
                      {msgUsers.map(u => (
                        <option key={u._id} value={u._id}>
                          {u.fullName} — {roleBadge(u.role).label}
                        </option>
                      ))}
                    </Select>
                  </Field>
                  ...
                  <Btn icon="fas fa-paper-plane" color="#1a3a5c" style={{
                    width: '100%', justifyContent: 'center', padding: '12px'
                  }}
                  onClick={async () => {
                    if (!selectedUser || !msgText.trim()) {
                      Swal.fire('Error', 'Select recipient and enter message', 'warning');
                      return;
                    }
                    await sendMessage();
          ...
          {activeTab === 'settings' && (
            <div style={{ maxWidth: 680, margin: '0 auto' }}>
              <div style={{
                ...
                <h2 style={{
                  margin: '0 0 20px', fontSize: 20, color: '#1a3a5c',
                  fontFamily: "'Crimson Text', Georgia, serif",
                  display: 'flex', alignItems: 'center', gap: 10
                }}>
                  <i className="fas fa-cog" style={{ color: '#ffc107' }} />
                  School Configuration
                </h2>

                <Field label="School Name" required>
                  <Input value={schoolSettings.schoolName}
                    onChange={e => setSchoolSettings({ ...schoolSettings, schoolName: e.target.value })} />
                </Field>
                <Field label="Motto">
                  <Input value={schoolSettings.motto}
                ...
                <div style={{
                  ...
                  marginTop: 16, borderTop: '1px solid #eee', paddingTop: 16
                }}>
                  <Btn onClick={() => setSettingsModal(false)} color="#f0f0f0" textColor="#666">
                    Cancel
                  </Btn>
                  <Btn onClick={updateSchoolSettings} icon="fas fa-save" color="#1a3a5c" disabled={saving}>
              ...
              <div style={{
                ...
                {[
                  ['Students', students.length, '#1abc9c', '#e0f7fa'],
                  ['Teachers', teachers.length, '#9b59b6', '#f3e5f5'],
                  ['Classes', classes.length, '#3498db', '#e3f2fd'],
                  ['Admins', admins.length, '#ffc107', '#fff8e1'],
                ].map(([l, v, c, bg]) => (
                  <div key={l} style={{
                    ...
                    <div style={{ fontSize: 11, color: c, opacity: .7 }}>{l}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'profile' && (
            <div style={{ maxWidth: 640, margin: '0 auto' }}>
              ...
              <div style={{
                ...
                boxShadow: '0 6px 24px rgba(26,58,92,.35)'
              }}>
                <Avatar name={userName} size={80} bg='rgba(255,193,7,.2)' color='#ffc107' />
                <h2 style={{
                  margin: '16px 0 4px', fontFamily: "'Crimson Text', Georgia, serif",
                  fontSize: 24
                }}>{userName}</h2>
              ...
              <div style={{
                background: 'white', borderRadius: 16, padding: '24px',
                marginBottom: 16, boxShadow: '0 2px 10px rgba(0,0,0,.06)'
              }}>
                <h3 style={{
                  margin: '0 0 18px', fontSize: 16, color: '#1a3a5c',
                  fontFamily: "'Crimson Text', Georgia, serif",
                  display: 'flex', alignItems: 'center', gap: 8
                }}>
      ...
      <Modal open={settingsModal} onClose={() => setSettingsModal(false)} title="School Settings" width={600}>
        <Field label="School Name" required>
          <Input value={schoolSettings.schoolName}
            onChange={e => setSchoolSettings({ ...schoolSettings, schoolName: e.target.value })} />
        </Field>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
          <Btn onClick={() => setSettingsModal(false)} color="#f0f0f0" textColor="#666">Cancel</Btn>
          <Btn onClick={updateSchoolSettings} icon="fas fa-save" color="#1a3a5c" disabled={saving}>
            {saving ? 'Saving…' : 'Save Settings'}
          </Btn>
        </div>
      ...
      <Modal open={massMessageModal} onClose={() => setMassMessageModal(false)} title="Mass Message" width={540}>
        <Field label="Subject" required>
          <Input value={massMessageForm.subject} placeholder="Message subject"
            onChange={e => setMassMessageForm(p => ({ ...p, subject: e.target.value }))} />
        </Field>
        <Field label="Audience" required>
          <Select value={massMessageForm.audience}
            onChange={e => setMassMessageForm(p => ({ ...p, audience: e.target.value }))}>
            <option value="all">📢 All Users</option>
        ...
        <Field label="Message" required>
          <Textarea value={massMessageForm.content} rows={6} placeholder="Type your message here..."
            onChange={e => setMassMessageForm(p => ({ ...p, content: e.target.value }))} />
        </Field>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
          <Btn onClick={() => setMassMessageModal(false)} color="#f0f0f0" textColor="#666">Cancel</Btn>
          <Btn onClick={sendMassMessage} icon="fas fa-envelope" color="#3498db" disabled={saving}>
            {saving ? 'Sending…' : 'Send Mass Message'}
          </Btn>
        </div>
      </Modal>
    </div>
